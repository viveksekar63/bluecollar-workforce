  ...
  try {
    ...
    await client.query(`
      ...
      SELECT
        ...
        CASE WHEN n % 10 < 7 THEN 80.2707 + (((n % 200) - 100) * 0.001) ELSE 76.9558 + (((n % 100) - 50) * 0.001) END,
        true, now()
      FROM generated
    `, [size]);
    ...
    await client.query(`
      WITH generated AS (
        SELECT md5('perf-worker-' || gs::text)::uuid AS worker_id, gs AS n
        FROM generate_series(1, $1::int) gs
      )
      INSERT INTO worker_work_preferences (id, "workerId", mobility, "willingToRelocate", "willingToTravel", "createdAt", "updatedAt")
      SELECT md5('perf-pref-' || n::text)::uuid, worker_id,
        CASE WHEN n % 5 = 0 THEN 'ANYWHERE_INDIA' ELSE 'LOCAL' END,
        n % 3 = 0, n % 4 = 0, now(), now()
      FROM generated
    `, [size]);
    ...
    await client.query(`
      WITH generated AS (
        ...
        WHERE name IN ('Electrical Wiring', 'Panel Installation', 'Industrial Electrical')
      )
      ...
      CROSS JOIN skills s
      WHERE g.n % 10 < 7
        AND (g.n % 4 <> 0 OR s.name = 'Electrical Wiring')
    `, [size]);
    ...
    await client.query(`
      ...
      WHERE g.n % 10 < 7
        AND (g.n % 6 <> 0 OR l.name = 'Tamil')
    `, [size]);

    await client.query('COMMIT');
    await client.query('ANALYZE "Worker"');
    await client.query('ANALYZE "WorkerSkill"');
    await client.query('ANALYZE "WorkerLanguage"');
    await client.query('ANALYZE "WorkerAddress"');
    await client.query('ANALYZE worker_work_preferences');
  } catch (error) {
    await client.query('ROLLBACK');
    throw error;
  }
  ...
  WHERE w.profession ILIKE '%Electrician%'
    ...
    AND EXISTS (
      ...
      WHERE wa."workerId" = w.id
        ...
        AND wa.latitude IS NOT NULL AND wa.longitude IS NOT NULL
...
skill_matches AS (
  ...
  FROM "WorkerSkill" ws
  JOIN worker_base wb ON wb.id = ws."workerId"
  WHERE ws."skillId" IN (
...
language_matches AS (
  ...
  JOIN worker_base wb ON wb.id = wl."workerId"
  JOIN "Language" l ON l.id = wl."languageId"
  WHERE l.name ILIKE '%Tamil%' OR l.name ILIKE '%English%'
  GROUP BY wl."workerId"
),
candidates AS (
  SELECT w.id,
    coalesce(sm."matchedSkillCount", 0) AS "matchedSkillCount",
    coalesce(lm."matchedLanguageCount", 0) AS "matchedLanguageCount",
    (
  ...
  WHERE coalesce(sm."matchedSkillCount", 0) = 3
    AND coalesce(lm."matchedLanguageCount", 0) = 2
)
SELECT id, "matchedSkillCount", "matchedLanguageCount", "distanceKm"
FROM candidates
ORDER BY "matchedSkillCount" DESC, "distanceKm" ASC, id ASC
LIMIT 50
`;
...
async function explain(client: any, sql: string): Promise<Sample> {
  ...
  return {
    ...
    sharedHitBlocks: Number(plan['Plan']?.['Shared Hit Blocks'] ?? 0),
    sharedReadBlocks: Number(plan['Plan']?.['Shared Read Blocks'] ?? 0),
  };
}

async function benchmark(client: any) {
  for (let i = 0; i < WARMUP; i++) await explain(client, productionLikeSql);

  const samples: Sample[] = [];
  for (let i = 0; i < ITERATIONS; i++) samples.push(await explain(client, productionLikeSql));

  const execution = samples.map((sample) => sample.executionMs);
  return {
    samples: execution.length,
    p50Ms: Number(percentile(execution, 50).toFixed(3)),
    p95Ms: Number(percentile(execution, 95).toFixed(3)),
    p99Ms: Number(percentile(execution, 99).toFixed(3)),
    minMs: Number(Math.min(...execution).toFixed(3)),
    maxMs: Number(Math.max(...execution).toFixed(3)),
    planningMs: Number(samples.at(-1)!.planningMs.toFixed(3)),
    executionMs: Number(samples.at(-1)!.executionMs.toFixed(3)),
    sharedHitBlocks: samples.at(-1)!.sharedHitBlocks,
    sharedReadBlocks: samples.at(-1)!.sharedReadBlocks,
  };
}
...
async function run() {
  ...
  try {
    console.log(`V5.3.1 scale benchmark: ${DATASET_SIZES.join(', ')} workers; ${ITERATIONS} samples; ${WARMUP} warmups`);
    ...
    for (const size of DATASET_SIZES) {
      ...
      console.log(`seedMs=${Date.now() - started}`);
      console.log(JSON.stringify({ datasetWorkers: size, ...(await benchmark(client)) }));
      await cleanup(client);
    }
  } finally {
    client.release();
