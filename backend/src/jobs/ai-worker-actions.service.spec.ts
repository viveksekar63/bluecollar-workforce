...
describe('AiWorkerActionsService', () => {
  ...
  beforeEach(() => {
    jest.clearAllMocks();
    service = new AiWorkerActionsService(prisma as any, jobsService as any);
  ...
  it('shortlists a verified employer-owned job and snapshots the AI match', async () => {
    ...
    const result = await service.shortlist('user-1', 'job-1', 'worker-1', {
      matchScore: 94,
      matchTier: 'BEST_MATCH',
      matchExplanation: { recommendation: 'Excellent fit' },
    });
  ...
  it('lists shortlisted workers with pagination', async () => {
    ...
    prisma.$queryRaw
      .mockResolvedValueOnce([{ workerId: 'worker-1', matchScore: 92 }])
  ...
  it('removes an existing shortlist', async () => {
    ...
    await expect(service.removeShortlist('user-1', 'job-1', 'worker-1')).resolves.toEqual({
      success: true,
  ...
  it('invites a worker only for a published job', async () => {
    prisma.job.findFirst.mockResolvedValue({ id: 'job-1', title: 'Electrician', status: 'PUBLISHED' });
    prisma.worker.findUnique.mockResolvedValue({ id: 'worker-1' });
    prisma.$queryRaw.mockResolvedValue([{ id: 'invite-1', invited_at: new Date('2026-09-07T11:00:00Z') }]);

    const result = await service.invite('user-1', 'job-1', 'worker-1', {
      matchScore: 88,
