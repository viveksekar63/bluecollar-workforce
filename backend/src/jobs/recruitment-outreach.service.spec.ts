...
describe('RecruitmentOutreachService', () => {
  ...
  it('initializes outreach only for shortlisted or invited workers', async () => {
    prisma.worker.findUnique.mockResolvedValue({ id: 'worker-1', userId: 'user-1' });
  ...
  it('logs a phone contact and creates a timeline event', async () => {
    ...
    prisma.$queryRaw.mockResolvedValueOnce([{ id: 'outreach-1' }]).mockResolvedValueOnce([{ outreachId: 'outreach-1', status: 'INTERESTED', contactAttempts: 1 }]);
