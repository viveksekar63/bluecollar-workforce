...
export class RecruitmentOutreachService {
  ...
  private validateChannel(channel: string): asserts channel is OutreachChannel {
    if (!['PHONE', 'WHATSAPP', 'SMS', 'EMAIL'].includes(channel)) throw new BadRequestException('channel must be PHONE, WHATSAPP, SMS, or EMAIL');
  }

  private validateStatus(status?: string): asserts status is OutreachStatus | undefined {
    if (status && !['NOT_CONTACTED','CONTACTED','NO_RESPONSE','INTERESTED','INTERVIEW','SELECTED','HIRED','NOT_INTERESTED','UNAVAILABLE','WRONG_NUMBER'].includes(status)) throw new BadRequestException('Invalid outreach status');
  }
  ...
  async list(userId: string, jobId: string, page = 1, limit = 20, status?: string) {
    ...
    const [items, countRows] = await Promise.all([
      this.prisma.$queryRaw<Array<Record<string, unknown>>>(Prisma.sql`
        SELECT o."id" AS "outreachId", c."workerId",
          COALESCE(o."status", 'NOT_CONTACTED') AS "status",
          o."preferred_channel" AS "preferredChannel", o."last_channel" AS "lastChannel",
          COALESCE(o."contact_attempts", 0) AS "contactAttempts", o."last_contacted_at" AS "lastContactedAt",
        ...
        FROM (${candidateSource}) c
        JOIN "Worker" w ON w."id" = c."workerId"
  ...
  async initialize(userId: string, jobId: string, workerId: string, preferredChannel?: OutreachChannel) {
    ...
    const worker = await this.prisma.worker.findUnique({ where: { id: workerId }, select: { id: true, userId: true } });
    if (!worker) throw new NotFoundException('Worker not found');
    const action = await this.prisma.$queryRaw<Array<{ id: string }>>(Prisma.sql`
      SELECT "id" FROM "job_worker_actions" WHERE "employer_id" = ${employer.id} AND "job_id" = ${job.id}
  ...
  async logContact(userId: string, jobId: string, workerId: string, input: LogContactInput) {
    const { employer, job } = await this.getOwnedJob(userId, jobId);
    this.validateChannel(input.channel); this.validateStatus(input.status);
    if (input.nextFollowUpAt && Number.isNaN(new Date(input.nextFollowUpAt).getTime())) throw new BadRequestException('nextFollowUpAt must be a valid ISO date');
    const worker = await this.prisma.worker.findUnique({ where: { id: workerId }, select: { id: true } });
    if (!worker) throw new NotFoundException('Worker not found');
    const action = await this.prisma.$queryRaw<Array<{ id: string }>>(Prisma.sql`
      SELECT "id" FROM "job_worker_actions" WHERE "employer_id" = ${employer.id} AND "job_id" = ${job.id} AND "worker_id" = ${workerId}
        AND "action_type" IN ('SHORTLISTED','INVITED') LIMIT 1
    `);
    if (!action[0]) throw new BadRequestException('Worker must be shortlisted or invited before outreach');
    const outreach = await this.prisma.$queryRaw<Array<{ id: string }>>(Prisma.sql`
      INSERT INTO "job_worker_outreach" ("id","employer_id","job_id","worker_id") VALUES (${randomUUID()},${employer.id},${job.id},${workerId})
      ON CONFLICT ("job_id","worker_id") DO NOTHING
      RETURNING "id"
    `);
    let outreachId = outreach[0]?.id;
    if (!outreachId) {
      const existing = await this.prisma.$queryRaw<Array<{ id: string }>>(Prisma.sql`SELECT "id" FROM "job_worker_outreach" WHERE "employer_id"=${employer.id} AND "job_id"=${job.id} AND "worker_id"=${workerId} LIMIT 1`);
      outreachId = existing[0]?.id;
    }
    if (!outreachId) throw new NotFoundException('Outreach record not found');
