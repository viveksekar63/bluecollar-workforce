...
export class MasterDataService {
  ...
  async categories(includeInactive = false) {
    if (includeInactive) return this.prisma.$queryRaw`
      SELECT "id", "code", "name", "isActive", "createdAt", "updatedAt" FROM "work_categories" ORDER BY "name" ASC`;
    return this.prisma.$queryRaw`
      SELECT "id", "code", "name", "isActive", "createdAt", "updatedAt" FROM "work_categories" WHERE "isActive" = true ORDER BY "name" ASC`;
  }
  ...
  async professions(categoryId?: string, includeInactive = false) {
    return this.prisma.$queryRaw`
      ...
      FROM "work_professions" p JOIN "work_categories" c ON c."id" = p."categoryId"
  ...
  async locations(type?: string, parentId?: string, includeInactive = false) {
    ...
    if (normalizedType && !['STATE', 'DISTRICT', 'CITY'].includes(normalizedType)) throw new BadRequestException('Invalid location type');
    return this.prisma.$queryRaw`
      SELECT l."id", l."type", l."name", l."code", l."parentId", l."pincode", l."isActive", p."name" AS "parentName"
      FROM "work_locations" l LEFT JOIN "work_locations" p ON p."id" = l."parentId"
  ...
  async createCategory(code: string, name: string) {
    this.requireText(code, 'Category code'); this.requireText(name, 'Category name');
    try { const rows = await this.prisma.$queryRaw<Array<any>>`
      INSERT INTO "work_categories" ("code", "name") VALUES (${code.trim().toUpperCase()}, ${name.trim()})
      RETURNING "id", "code", "name", "isActive"`; return rows[0]; }
    catch { throw new BadRequestException('Category code or name already exists'); }
  }

  async createProfession(categoryId: string, code: string, name: string) {
    this.requireText(categoryId, 'Category'); this.requireText(code, 'Profession code'); this.requireText(name, 'Profession name');
    ...
    try { const rows = await this.prisma.$queryRaw<Array<any>>`
      INSERT INTO "work_professions" ("categoryId", "code", "name") VALUES (${categoryId}, ${code.trim().toUpperCase()}, ${name.trim()})
      RETURNING "id", "categoryId", "code", "name", "isActive"`; return rows[0]; }
    catch { throw new BadRequestException('Profession code or category/name already exists'); }
  }
  ...
  async createLocation(type: string, name: string, parentId?: string, code?: string, pincode?: string) {
    ...
    try { const rows = await this.prisma.$queryRaw<Array<any>>`
      ...
      RETURNING "id", "type", "name", "code", "parentId", "pincode", "isActive"`; return rows[0]; }
    catch { throw new BadRequestException('Location already exists at this level'); }
  }

  async setActive(kind: 'category' | 'profession' | 'location', id: string, isActive: boolean) {
    if (kind === 'category') {
      const rows = await this.prisma.$queryRaw<Array<{ id: string }>>`SELECT "id" FROM "work_categories" WHERE "id"=${id} LIMIT 1`;
      if (!rows[0]) throw new NotFoundException('category not found');
      await this.prisma.$executeRaw`UPDATE "work_categories" SET "isActive"=${isActive}, "updatedAt"=CURRENT_TIMESTAMP WHERE "id"=${id}`;
    } else if (kind === 'profession') {
      const rows = await this.prisma.$queryRaw<Array<{ id: string }>>`SELECT "id" FROM "work_professions" WHERE "id"=${id} LIMIT 1`;
      if (!rows[0]) throw new NotFoundException('profession not found');
      await this.prisma.$executeRaw`UPDATE "work_professions" SET "isActive"=${isActive}, "updatedAt"=CURRENT_TIMESTAMP WHERE "id"=${id}`;
    } else {
      const rows = await this.prisma.$queryRaw<Array<{ id: string }>>`SELECT "id" FROM "work_locations" WHERE "id"=${id} LIMIT 1`;
      if (!rows[0]) throw new NotFoundException('location not found');
      await this.prisma.$executeRaw`UPDATE "work_locations" SET "isActive"=${isActive}, "updatedAt"=CURRENT_TIMESTAMP WHERE "id"=${id}`;
    }
