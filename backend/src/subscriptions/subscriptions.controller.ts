...
class VerifySubscriptionDto {
  ...
  @IsString() razorpaySubscriptionId!: string;
...
export class SubscriptionsController {
  ...
  plans() { return this.subscriptionsService.listPlans(); }

  @Get('current')
  @UseGuards(JwtAuthGuard)
  current(@CurrentUser() user: { userId: string }) { return this.subscriptionsService.current(user.userId); }

  @Post()
  @UseGuards(JwtAuthGuard)
  create(@CurrentUser() user: { userId: string }, @Body() dto: CreateSubscriptionDto) {
    return this.subscriptionsService.create(user.userId, dto.planCode);
  }

  @Post('verify')
  @UseGuards(JwtAuthGuard)
  verify(@CurrentUser() user: { userId: string }, @Body() dto: VerifySubscriptionDto) {
    return this.subscriptionsService.verifyCheckout(user.userId, dto);
  }

  @Post('cancel')
  @UseGuards(JwtAuthGuard)
  cancel(@CurrentUser() user: { userId: string }) { return this.subscriptionsService.cancelAtPeriodEnd(user.userId); }
