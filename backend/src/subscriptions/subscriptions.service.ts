...
type PlanRow = {
  ...
  priceInr: number; currency: string; billingInterval: string;
  jobLimit: number; razorpayPlanId: string | null;
};

type SubscriptionRow = {
  id: string; employerId: string; planId: string; razorpaySubscriptionId: string;
  status: string; currentPeriodStart: Date | null; currentPeriodEnd: Date | null;
  jobsUsed: number; jobLimit: number; cancelAtPeriodEnd: boolean; endedAt: Date | null;
  planCode: string; planName: string; priceInr: number; billingInterval: string;
};
...
export class SubscriptionsService {
  ...
  async listPlans() {
    return this.prisma.$queryRaw<PlanRow[]>`
      SELECT "id", "code", "name", "description", "priceInr", "currency", "billingInterval", "jobLimit", "razorpayPlanId"
      FROM "employer_subscription_plans"
      WHERE "isActive" = true AND "razorpayPlanId" IS NOT NULL
  ...
  private async getCurrentForEmployer(employerId: string) {
    const rows = await this.prisma.$queryRaw<SubscriptionRow[]>`
      SELECT s."id", s."employerId", s."planId", s."razorpaySubscriptionId", s."status",
        s."currentPeriodStart", s."currentPeriodEnd", s."jobsUsed", s."jobLimit",
        s."cancelAtPeriodEnd", s."endedAt", p."code" AS "planCode", p."name" AS "planName",
  ...
  async current(userId: string) {
    ...
    const subscription = await this.getCurrentForEmployer(employer.id);
    ...
    const active = subscription.status === 'active'
      ...
      && subscription.jobsUsed < subscription.jobLimit;
  ...
  async create(userId: string, planCode: string) {
    ...
    const planRows = await this.prisma.$queryRaw<PlanRow[]>`
      SELECT "id", "code", "name", "description", "priceInr", "currency", "billingInterval", "jobLimit", "razorpayPlanId"
      FROM "employer_subscription_plans"
      WHERE "code" = ${planCode.trim().toUpperCase()} AND "isActive" = true LIMIT 1
    `;
    ...
    const existing = await this.getCurrentForEmployer(employer.id);
    if (existing) throw new BadRequestException('You already have an employer subscription. Manage the current subscription before starting another one.');
    ...
    try {
      ...
      await this.prisma.$executeRaw`
        INSERT INTO "employer_subscriptions"
          ("id", "employerId", "planId", "razorpaySubscriptionId", "status", "currentPeriodStart", "currentPeriodEnd", "jobsUsed", "jobLimit", "updatedAt")
        VALUES (${randomUUID()}, ${employer.id}, ${plan.id}, ${subscription.id}, ${String(subscription.status ?? 'created')},
          ${subscription.current_start ? new Date(Number(subscription.current_start) * 1000) : null},
          ${subscription.current_end ? new Date(Number(subscription.current_end) * 1000) : null}, 0, ${plan.jobLimit}, CURRENT_TIMESTAMP)
        ON CONFLICT ("razorpaySubscriptionId") DO UPDATE SET "status" = EXCLUDED."status", "currentPeriodStart" = EXCLUDED."currentPeriodStart", "currentPeriodEnd" = EXCLUDED."currentPeriodEnd", "updatedAt" = CURRENT_TIMESTAMP
      `;
      return {
        keyId,
        subscriptionId: subscription.id as string,
        shortUrl: subscription.short_url as string | null,
        status: String(subscription.status ?? 'created'),
        plan: { code: plan.code, name: plan.name, priceInr: plan.priceInr, currency: plan.currency, billingInterval: plan.billingInterval, jobLimit: plan.jobLimit },
      };
    } catch (error: any) {
      throw new BadRequestException(error?.response?.data?.error?.description || 'Unable to create Razorpay subscription');
    }
  }
  ...
  async verifyCheckout(userId: string, input: { razorpayPaymentId: string; razorpaySubscriptionId: string; razorpaySignature: string }) {
    ...
    if (expectedBuffer.length !== receivedBuffer.length || !timingSafeEqual(expectedBuffer, receivedBuffer)) throw new BadRequestException('Subscription payment signature verification failed');

    const owned = await this.prisma.$queryRaw<Array<{ id: string }>>`
      SELECT "id" FROM "employer_subscriptions" WHERE "employerId" = ${employer.id} AND "razorpaySubscriptionId" = ${subscriptionId} LIMIT 1
    `;
    ...
    try {
      ...
      return { verified: true, razorpayPaymentId: paymentId, subscriptionId, status: String(response.data.status), active: String(response.data.status) === 'active' };
    } catch (error: any) {
      throw new BadRequestException(error?.response?.data?.error?.description || 'Unable to verify subscription with Razorpay');
    }
  }
  ...
  private async syncRazorpaySubscription(subscription: any) {
    ...
    await this.prisma.$executeRaw`
      ...
      SET "status" = ${status}, "currentPeriodStart" = ${currentStart}, "currentPeriodEnd" = ${currentEnd}, "endedAt" = ${endedAt},
        "cancelAtPeriodEnd" = CASE WHEN ${status} IN ('cancelled', 'completed', 'expired') THEN false ELSE "cancelAtPeriodEnd" END,
  ...
  async handleWebhook(rawBody: Buffer, signature: string | undefined, eventId: string | undefined) {
    ...
    const expectedBuffer = Buffer.from(expected, 'utf8');
    const receivedBuffer = Buffer.from(signature, 'utf8');
    if (expectedBuffer.length !== receivedBuffer.length || !timingSafeEqual(expectedBuffer, receivedBuffer)) throw new BadRequestException('Invalid Razorpay webhook signature');
    ...
    if (!eventName.startsWith('subscription.')) return { received: true, ignored: true };

    return this.prisma.$transaction(async (tx) => {
      const inserted = await tx.$queryRaw<Array<{ id: string }>>`
        INSERT INTO "employer_subscription_webhook_events" ("id", "eventId", "event", "payload")
  ...
  async reserveJobPublishSlot(employerId: string) {
    const rows = await this.prisma.$queryRaw<Array<{ id: string; jobsUsed: number; jobLimit: number; currentPeriodEnd: Date | null }>>`
      UPDATE "employer_subscriptions" SET "jobsUsed" = "jobsUsed" + 1, "updatedAt" = CURRENT_TIMESTAMP
      WHERE "id" = (
        SELECT s."id" FROM "employer_subscriptions" s
        WHERE s."employerId" = ${employerId} AND s."status" = 'active'
          AND (s."currentPeriodEnd" IS NULL OR s."currentPeriodEnd" > CURRENT_TIMESTAMP)
          AND s."jobsUsed" < s."jobLimit"
        ORDER BY s."createdAt" DESC LIMIT 1 FOR UPDATE
      )
  ...
  async releaseJobPublishSlot(employerId: string) {
    await this.prisma.$executeRaw`
      UPDATE "employer_subscriptions" SET "jobsUsed" = GREATEST("jobsUsed" - 1, 0), "updatedAt" = CURRENT_TIMESTAMP
      WHERE "id" = (SELECT "id" FROM "employer_subscriptions" WHERE "employerId" = ${employerId} AND "status" = 'active' ORDER BY "createdAt" DESC LIMIT 1)
    `;
  }
  ...
  async cancelAtPeriodEnd(userId: string) {
    ...
    const { keyId, keySecret } = this.razorpayConfig();
    try {
      const response = await axios.post(`https://api.razorpay.com/v1/subscriptions/${current.razorpaySubscriptionId}/cancel`, { cancel_at_cycle_end: true }, { auth: { username: keyId, password: keySecret } });
      await this.syncRazorpaySubscription(response.data);
      ...
      throw new BadRequestException(error?.response?.data?.error?.description || 'Unable to cancel subscription');
    }
  }
}
