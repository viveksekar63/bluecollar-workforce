...
export class SubscriptionsService {
  ...
  async listPlans() {
    return this.prisma.$queryRaw<PlanRow[]>`SELECT "id", "code", "name", "description", "priceInr", "currency", "billingInterval", "jobLimit", "razorpayPlanId" FROM "employer_subscription_plans" WHERE "isActive" = true AND "razorpayPlanId" IS NOT NULL ORDER BY "priceInr" ASC`;
  }
  ...
  async create(userId: string, planCode: string) {
    ...
    const planRows = await this.prisma.$queryRaw<PlanRow[]>`SELECT "id", "code", "name", "description", "priceInr", "currency", "billingInterval", "jobLimit", "razorpayPlanId" FROM "employer_subscription_plans" WHERE "code" = ${planCode.trim().toUpperCase()} AND "isActive" = true LIMIT 1`;
    ...
    if (await this.getCurrentForEmployer(employer.id)) throw new BadRequestException('You already have an employer subscription. Manage the current subscription before starting another one.');
    ...
    try {
      ...
      await this.prisma.$executeRaw`INSERT INTO "employer_subscriptions" ("id", "employerId", "planId", "razorpaySubscriptionId", "status", "currentPeriodStart", "currentPeriodEnd", "jobsUsed", "jobLimit", "updatedAt") VALUES (${randomUUID()}, ${employer.id}, ${plan.id}, ${subscription.id}, ${String(subscription.status ?? 'created')}, ${subscription.current_start ? new Date(Number(subscription.current_start) * 1000) : null}, ${subscription.current_end ? new Date(Number(subscription.current_end) * 1000) : null}, 0, ${plan.jobLimit}, CURRENT_TIMESTAMP) ON CONFLICT ("razorpaySubscriptionId") DO UPDATE SET "status" = EXCLUDED."status", "currentPeriodStart" = EXCLUDED."currentPeriodStart", "currentPeriodEnd" = EXCLUDED."currentPeriodEnd", "updatedAt" = CURRENT_TIMESTAMP`;
      return { keyId, subscriptionId: subscription.id as string, shortUrl: subscription.short_url as string | null, status: String(subscription.status ?? 'created'), plan: { code: plan.code, name: plan.name, priceInr: plan.priceInr, currency: plan.currency, billingInterval: plan.billingInterval, jobLimit: plan.jobLimit } };
  ...
  async reserveJobPublishSlot(employerId: string) {
    ...
    if (!rows[0]) { const current = await this.getCurrentForEmployer(employerId); if (!current) throw new BadRequestException('SUBSCRIPTION_REQUIRED: Choose an employer subscription to publish jobs.'); if (current.status !== 'active') throw new BadRequestException(`SUBSCRIPTION_INACTIVE: Your subscription is ${current.status}. Complete payment or update your payment method.`); throw new BadRequestException(`JOB_LIMIT_REACHED: You have used all ${current.jobLimit} job postings in the current billing period.`); }
    return rows[0];
  }
  ...
  async cancelAtPeriodEnd(userId: string) {
    const employer = await this.getEmployer(userId); const current = await this.getCurrentForEmployer(employer.id); if (!current) throw new NotFoundException('No active subscription found'); const { keyId, keySecret } = this.razorpayConfig();
