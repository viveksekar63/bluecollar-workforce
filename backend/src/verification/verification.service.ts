...
export class VerificationService {
  ...
  async create(workerId: string, input: CreateVerificationRequestInput = {}) {
    const worker = await this.prisma.worker.findUnique({ where: { id: workerId }, select: { id: true } });
    if (!worker) throw new NotFoundException("Worker not found");
    ...
    const existing = await this.prisma.verificationRequest.findFirst({
      where: {
        workerId,
        status: { in: [VerificationStatus.PENDING, VerificationStatus.IN_PROGRESS] },
      },
      orderBy: { createdAt: "desc" },
    });

    if (existing) throw new BadRequestException("Worker already has an active verification request");

    const types = input.types?.length ? [...new Set(input.types)] : Object.values(VerificationType);
  ...
  async updateStatus(id: string, input: UpdateVerificationStatusInput) {
    ...
    if (
      ...
      ![VerificationStatus.PENDING, VerificationStatus.MANUAL_REVIEW].includes(verification.status)
    ) {
      throw new BadRequestException("Verification can only be started from PENDING or MANUAL_REVIEW");
    }

    const now = new Date();
    const completed = [VerificationStatus.VERIFIED, VerificationStatus.FAILED, VerificationStatus.EXPIRED].includes(input.status);

    return this.prisma.$transaction(async (tx) => {
      const score = input.status === VerificationStatus.VERIFIED ? await this.calculateScore(id, tx) : undefined;
      const updated = await tx.verificationRequest.update({
        where: { id },
        data: {
          status: input.status,
          startedAt: input.status === VerificationStatus.IN_PROGRESS ? now : undefined,
  ...
  async updateCheck(checkId: string, input: UpdateVerificationCheckInput) {
    ...
    return this.prisma.$transaction(async (tx) => {
      ...
      const updatedCheck = await tx.verificationCheck.update({
        ...
        data: {
          status: input.status,
          startedAt: input.status === VerificationStatus.IN_PROGRESS ? new Date() : undefined,
  ...
  private async refreshRequestStatus(verificationId: string, tx: any) {
    ...
    const statuses = request.checks.map((check: { status: VerificationStatus }) => check.status);
    let status = VerificationStatus.IN_PROGRESS;

    if (statuses.every((value: VerificationStatus) => value === VerificationStatus.VERIFIED)) {
      status = VerificationStatus.VERIFIED;
    } else if (statuses.some((value: VerificationStatus) => value === VerificationStatus.FAILED)) {
      status = VerificationStatus.FAILED;
    } else if (statuses.some((value: VerificationStatus) => value === VerificationStatus.MANUAL_REVIEW)) {
      status = VerificationStatus.MANUAL_REVIEW;
    } else if (statuses.every((value: VerificationStatus) => value === VerificationStatus.PENDING)) {
      status = VerificationStatus.PENDING;
    }

    const score = await this.calculateScore(verificationId, tx);
    const completed = [VerificationStatus.VERIFIED, VerificationStatus.FAILED].includes(status);
    ...
    await tx.verificationRequest.update({
      ...
      data: {
        status,
        overallScore: score,
        startedAt: status !== VerificationStatus.PENDING ? undefined : null,
        completedAt: completed ? new Date() : null,
      },
    });
  ...
  private async calculateScore(verificationId: string, tx: any): Promise<number | null> {
    const results = await tx.verificationResult.findMany({
      where: { verificationCheck: { verificationId }, score: { not: null } },
      select: { score: true },
    });
    if (!results.length) return null;
    const total = results.reduce((sum: number, item: { score: number | null }) => sum + (item.score ?? 0), 0);
