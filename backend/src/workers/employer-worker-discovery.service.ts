  ...
  async findAll(query: WorkersQueryDto) {
    ...
    if (skill) {
      filters.push(Prisma.sql`EXISTS (
        ...
        WHERE ws."workerId" = w."id" AND sk."name" ILIKE ${`%${skill}%`}
      )`);
    }
    ...
    const preferredLocationMatches = (parts: Prisma.Sql[]) => Prisma.sql`EXISTS (
      ...
      WHERE pl."workerId" = w."id" AND (${Prisma.join(parts, ' AND ')})
    )`;
    ...
    if (hierarchyParts.length) {
      ...
      filters.push(Prisma.sql`(${currentAddressMatches(hierarchyParts)} OR ${preferredLocationMatches(preferredParts)} OR EXISTS (
        SELECT 1 FROM "worker_work_preferences" wp_hierarchy
        WHERE wp_hierarchy."workerId" = w."id" AND wp_hierarchy."mobility" = 'ANYWHERE_INDIA'
      ))`);
    }
    ...
    if (freeTextLocation) {
      ...
      filters.push(Prisma.sql`(
        EXISTS (SELECT 1 FROM "WorkerAddress" wa WHERE wa."workerId" = w."id" AND wa."isCurrent" = true
          AND (wa."city" ILIKE ${locationPattern} OR COALESCE(wa."district", '') ILIKE ${locationPattern} OR wa."state" ILIKE ${locationPattern} OR wa."pincode" ILIKE ${locationPattern}))
        OR EXISTS (SELECT 1 FROM "worker_preferred_locations" pl WHERE pl."workerId" = w."id"
          AND (pl."city" ILIKE ${locationPattern} OR COALESCE(pl."district", '') ILIKE ${locationPattern} OR pl."state" ILIKE ${locationPattern}))
    ...
    if (search) {
      ...
      filters.push(Prisma.sql`(
        w."workerCode" ILIKE ${pattern} OR u."firstName" ILIKE ${pattern} OR COALESCE(u."lastName", '') ILIKE ${pattern}
        OR COALESCE(u."email", '') ILIKE ${pattern} OR u."phone" ILIKE ${pattern} OR COALESCE(w."bio", '') ILIKE ${pattern}
        OR COALESCE(w."profession", '') ILIKE ${pattern} OR COALESCE(w."professionCategory", '') ILIKE ${pattern}
        OR EXISTS (SELECT 1 FROM "WorkerSkill" ws2 JOIN "Skill" sk2 ON sk2."id" = ws2."skillId" WHERE ws2."workerId" = w."id" AND sk2."name" ILIKE ${pattern})
        OR EXISTS (SELECT 1 FROM "WorkerAddress" wa2 WHERE wa2."workerId" = w."id" AND wa2."isCurrent" = true AND (wa2."city" ILIKE ${pattern} OR wa2."state" ILIKE ${pattern}))
        OR EXISTS (SELECT 1 FROM "worker_preferred_locations" pl2 WHERE pl2."workerId" = w."id" AND (pl2."city" ILIKE ${pattern} OR pl2."state" ILIKE ${pattern}))
      )`);
    }
    ...
    const [{ count }] = await this.prisma.$queryRaw<Array<{ count: bigint }>>(Prisma.sql`
      SELECT COUNT(*)::bigint AS count FROM "Worker" w JOIN "User" u ON u."id" = w."userId" ${where}
    `);
    const total = Number(count);

    return {
      items: rows.map((worker) => ({
        id: worker.id, workerCode: worker.workerCode, firstName: worker.firstName, lastName: worker.lastName ?? '',
        profileImageUrl: worker.profileImageUrl, primarySkill: worker.primarySkill ?? 'Not specified',
        skills: Array.isArray(worker.workerSkills) ? worker.workerSkills : [],
        professionCategory: worker.professionCategory, profession: worker.profession,
        experienceYears: Number(worker.experienceYears ?? 0), city: worker.city ?? 'Not specified',
        district: worker.district ?? 'Not specified', state: worker.state ?? 'Not specified',
        verificationScore: worker.verificationScore ?? 0, verificationStatus: worker.verificationStatus,
