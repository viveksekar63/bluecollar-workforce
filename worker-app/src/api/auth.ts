...
export async function login(identifier:string,password:string,role:MobileRole){const {data}=await api.post<AuthResponse>('/auth/login',{identifier,password,role});setAccessToken(data.accessToken);return data;}
...
export async function verifyLoginOtp(phone:string,otp:string,role:MobileRole){const {data}=await api.post<AuthResponse & {success:boolean;role:MobileRole;requiresApproval:boolean;message:string}>('/auth/login/verify-otp',{phone,otp,role});setAccessToken(data.accessToken);return data;}
...
export async function verifyRegistrationOtp(phone:string,otp:string){const {data}=await api.post<RegistrationVerifyResponse>('/auth/register/verify-otp',{phone,otp});setAccessToken(data.accessToken);return data;}
export async function registerWorker(input:Omit<RegisterInput,'role'|'companyName'>){const {data}=await api.post<AuthResponse>('/auth/worker/register',input);setAccessToken(data.accessToken);return data;}
export function logoutWorker(){setAccessToken(null);}
