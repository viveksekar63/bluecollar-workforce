...
export type EmployerSubscriptionPlan = { id: string; code: string; name: string; description?: string | null; priceInr: number; currency: string; billingInterval: string; jobLimit: number; razorpayPlanId?: string | null };
export type EmployerSubscription = { id: string; employerId: string; planId: string; razorpaySubscriptionId?: string | null; status: string; currentPeriodStart?: string | null; currentPeriodEnd?: string | null; jobsUsed: number; jobLimit: number; cancelAtPeriodEnd: boolean; endedAt?: string | null; planCode: string; planName: string; priceInr: number; billingInterval: string };
...
export async function getCurrentSubscription() {
  // Sync first so local development also works when Razorpay cannot reach localhost webhooks.
  await api.post('/subscriptions/sync').catch(() => undefined);
  const response = await api.get<{ active: boolean; subscription: EmployerSubscription | null }>('/subscriptions/current');
...
export async function createEmployerSubscription(planCode: string) { const response = await api.post<{ keyId: string | null; subscriptionId: string | null; shortUrl: string | null; status: string; active: boolean; plan: EmployerSubscriptionPlan }>('/subscriptions', { planCode }); return response.data; }
