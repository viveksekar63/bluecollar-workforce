...
// Lightweight local SVG illustration. Keeping the artwork in the app avoids a remote-image dependency
// while giving the hero the same blue illustrated feel as the approved Indigo & Sky Blue design.
...
const bannerImage = { uri: `data:image/svg+xml;charset=utf-8,${bannerSvg}` };
  ...
  return <SafeAreaView style={styles.container}>
    <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={styles.content} refreshControl={<RefreshControl refreshing={refreshing} onRefresh={() => { setRefreshing(true); void load(); }} tintColor={BrandColors.indigo} />}>
      ...
      <View style={styles.hero}>
        <Image source={bannerImage} style={styles.heroArt} resizeMode="cover" />
...
function Stat({ icon, value, label }: { icon:string; value:number; label:React.ReactNode }) { return <View style={styles.stat}><View style={styles.statIcon}><Text style={styles.statIconText}>{icon}</Text></View><Text style={styles.statValue}>{value}</Text><Text style={styles.statLabel}>{label}</Text></View>; }
