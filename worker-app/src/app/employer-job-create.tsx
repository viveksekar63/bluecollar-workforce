...
export default function EmployerJobCreateScreen() {
  ...
  async function save() {
    ...
    if (!skillNames.trim()) {
      Alert.alert('Add required skills', 'Please add at least one skill, for example: Cooking, Parotta, Kitchen work.');
      return;
    }
  ...
  return <SafeAreaView style={styles.container}><ScrollView contentContainerStyle={styles.content} keyboardShouldPersistTaps="handled">
    ...
    <Field label="Required skills" value={skillNames} onChangeText={setSkillNames} placeholder="e.g. Cooking, Parotta, Kitchen work" />
    <Text style={styles.helper}>Separate multiple skills with commas.</Text>
...
const styles = StyleSheet.create({
  ...
  helper: { color: BrandColors.muted, fontSize: 10, marginTop: 5 },
