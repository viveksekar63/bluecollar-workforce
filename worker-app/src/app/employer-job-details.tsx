...
export default function EmployerJobDetailsScreen() {
  ...
  async function saveSkills() {
    if (!id || !skillNames.trim()) {
      Alert.alert('Add required skills', 'Please add at least one skill.');
      return;
    }
    try {
      setSavingSkills(true);
      const updated = await updateEmployerJob(String(id), {
        skillNames: skillNames.split(',').map((skill) => skill.trim()).filter(Boolean),
      });
      setJob(updated);
      setSkillNames(updated.skills?.filter((item) => item.required).map((item) => item.skill.name).join(', ') ?? skillNames);
      Alert.alert('Skills saved', 'Required skills have been updated.');
    } catch (e: any) {
  ...
  async function publish() {
    if (!id) return;
    const hasSkills = Boolean(job?.skills?.some((skill) => skill.required));
    if (!hasSkills) {
  ...
  const canPublish = job.status === 'DRAFT' && Boolean(job.skills?.some((skill) => skill.required));
  ...
  return <SafeAreaView style={styles.container}><ScrollView refreshControl={<RefreshControl refreshing={refreshing} onRefresh={() => { setRefreshing(true); void load(); }} tintColor={BrandColors.gold} />} contentContainerStyle={styles.content}>
    ...
    {job.status === 'DRAFT' && <View style={styles.card}>
      ...
      <Text style={styles.skillHint}>Add the skills workers must have. Separate multiple skills with commas.</Text>
      <TextInput value={skillNames} onChangeText={setSkillNames} placeholder="Cooking, Parotta, Kitchen work" placeholderTextColor={BrandColors.muted} style={styles.skillInput} />
...
const styles = StyleSheet.create({
  ...
  card: { marginTop: 16, backgroundColor: BrandColors.slateSoft, borderColor: BrandColors.slateBorder, borderWidth: 1, borderRadius: 18, padding: 16 }, sectionTitle: { color: BrandColors.text, fontSize: 16, fontWeight: '900' }, description: { color: BrandColors.textSecondary, lineHeight: 21, marginTop: 8 }, meta: { color: BrandColors.textSecondary, fontSize: 12, marginTop: 8 },
  skillHint: { color: BrandColors.textSecondary, fontSize: 11, lineHeight: 17, marginTop: 6 }, skillInput: { marginTop: 12, minHeight: 50, borderRadius: 13, borderWidth: 1, borderColor: BrandColors.slateBorder, backgroundColor: BrandColors.slate, paddingHorizontal: 12, color: BrandColors.text, fontSize: 14 },
  secondary: { marginTop: 10, height: 44, borderRadius: 12, borderWidth: 1, borderColor: BrandColors.gold, alignItems: 'center', justifyContent: 'center' }, secondaryText: { color: BrandColors.gold, fontWeight: '900' },
  warning: { marginTop: 12, padding: 12, borderRadius: 13, backgroundColor: '#2A2416', borderWidth: 1, borderColor: '#5C4A20' }, warningText: { color: '#F8D77A', fontSize: 11, fontWeight: '700' },
  primary: { marginTop: 14, height: 52, borderRadius: 14, backgroundColor: BrandColors.gold, alignItems: 'center', justifyContent: 'center' }, disabled: { opacity: 0.45 }, primaryText: { color: BrandColors.slate, fontWeight: '900' },
