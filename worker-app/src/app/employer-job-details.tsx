...
export default function EmployerJobDetailsScreen() {
  ...
  const smartSuggestions = useMemo(
    () => getSmartSkillSuggestions(job?.title ?? '', job?.description ?? '', skillNames),
    [job?.title, job?.description, skillNames],
  );
  ...
  async function saveSkills() {
    ...
    if (!id || names.length === 0) {
      Alert.alert('Add required skills', 'Please add at least one skill.');
      return;
    }
    ...
    try {
      ...
      setJob({ ...updated, skills: updated.skills?.length ? updated.skills : localSkills });
      setSkillNames(names.join(', '));
      Alert.alert('Skills saved', 'Required skills have been updated. You can publish the job now.');
    } catch (e: any) {
      Alert.alert('Unable to save skills', e?.response?.data?.message ?? 'Please try again.');
  ...
  async function publish() {
    ...
    if (!hasSkills) {
      Alert.alert('Add required skills', 'Add at least one required skill before publishing.');
      return;
    }
  ...
  return <SafeAreaView style={styles.container}><ScrollView refreshControl={<RefreshControl refreshing={refreshing} onRefresh={() => { setRefreshing(true); void load(); }} tintColor={BrandColors.gold} />} contentContainerStyle={styles.content} keyboardShouldPersistTaps="handled">
    ...
    {job.status === 'DRAFT' && <View style={styles.card}>
      <Text style={styles.sectionTitle}>Required skills</Text>
      <Text style={styles.skillHint}>Select suggested skills or search/add your own.</Text>
      <TextInput value={skillNames} onChangeText={setSkillNames} onFocus={() => setSkillFocused(true)} placeholder="Cooking, Parotta, Kitchen work" placeholderTextColor={BrandColors.muted} style={styles.skillInput} />
      ...
      {skillFocused && smartSuggestions.length > 0 && <View style={styles.suggestionPanel}>
        ...
        <View style={styles.chips}>
          {smartSuggestions.map((skill) => <Pressable key={`suggested-${skill}`} onPress={() => selectSkill(skill)} style={styles.suggestionChip}><Text style={styles.suggestionChipText}>+ {skill}</Text></Pressable>)}
        </View>
      </View>}

      <Text style={styles.predefinedTitle}>Popular predefined skills</Text>
      <View style={styles.chips}>
        {PREDEFINED_SKILLS.slice(0, 10).map((skill) => <Pressable key={skill} onPress={() => selectSkill(skill)} style={styles.predefinedChip}><Text style={styles.predefinedChipText}>{skill}</Text></Pressable>)}
      </View>
...
const styles = StyleSheet.create({
  ...
  title: { color: BrandColors.text, fontSize: 26, fontWeight: '900' },
  subtitle: { color: BrandColors.textSecondary, marginTop: 4, fontSize: 13 },
  statusRow: { flexDirection: 'row', alignItems: 'center', gap: 10, marginTop: 15 },
  badge: { borderWidth: 1, borderColor: BrandColors.gold, borderRadius: 999, paddingHorizontal: 10, paddingVertical: 5 },
  badgeText: { color: BrandColors.gold, fontSize: 10, fontWeight: '900' },
  location: { color: BrandColors.textSecondary, fontSize: 12 },
  card: { marginTop: 16, backgroundColor: BrandColors.slateSoft, borderColor: BrandColors.slateBorder, borderWidth: 1, borderRadius: 18, padding: 16 },
  sectionTitle: { color: BrandColors.text, fontSize: 16, fontWeight: '900' },
  description: { color: BrandColors.textSecondary, lineHeight: 21, marginTop: 8 },
  meta: { color: BrandColors.textSecondary, fontSize: 12, marginTop: 8 },
  skillHint: { color: BrandColors.textSecondary, fontSize: 11, lineHeight: 17, marginTop: 6 },
  skillInput: { marginTop: 12, minHeight: 50, borderRadius: 13, borderWidth: 1, borderColor: BrandColors.slateBorder, backgroundColor: BrandColors.slate, paddingHorizontal: 12, color: BrandColors.text, fontSize: 14 },
  suggestionPanel: { marginTop: 9, padding: 10, borderRadius: 13, backgroundColor: BrandColors.slate, borderWidth: 1, borderColor: BrandColors.slateBorder },
  suggestionTitle: { color: BrandColors.gold, fontSize: 10, fontWeight: '900', marginBottom: 8 },
  ...
  secondary: { marginTop: 12, height: 44, borderRadius: 12, borderWidth: 1, borderColor: BrandColors.gold, alignItems: 'center', justifyContent: 'center' },
  secondaryText: { color: BrandColors.gold, fontWeight: '900' },
  warning: { marginTop: 12, padding: 12, borderRadius: 13, backgroundColor: '#2A2416', borderWidth: 1, borderColor: '#5C4A20' },
  warningText: { color: '#F8D77A', fontSize: 11, fontWeight: '700' },
  primary: { marginTop: 14, height: 52, borderRadius: 14, backgroundColor: BrandColors.gold, alignItems: 'center', justifyContent: 'center' },
  ...
  link: { color: BrandColors.gold, fontWeight: '900', fontSize: 12 },
  application: { marginTop: 10, backgroundColor: BrandColors.slate, borderWidth: 1, borderColor: BrandColors.slateBorder, borderRadius: 14, padding: 14 },
  name: { color: BrandColors.text, fontWeight: '900', fontSize: 14 },
  appMeta: { color: BrandColors.textSecondary, fontSize: 11, marginTop: 4 },
  emptyText: { color: BrandColors.textSecondary, marginTop: 10 },
  emptyTitle: { color: BrandColors.text, fontSize: 18, fontWeight: '900' },
});
