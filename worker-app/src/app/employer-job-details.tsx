...
export default function EmployerJobDetailsScreen() {
  ...
  async function publish() {
    ...
    try {
      ...
      if (!current.active) {
        router.push({ pathname: '/employer-subscription', params: { jobId: String(id) } });
        return;
      }
      ...
      if (String(message).includes('SUBSCRIPTION_REQUIRED') || String(message).includes('SUBSCRIPTION_INACTIVE')) {
        router.push({ pathname: '/employer-subscription', params: { jobId: String(id) } });
      } else {
