...
export default function EmployerSubscriptionScreen() {
  ...
  async function refreshStatus() {
    ...
    try {
      ...
      if (current.active && jobId) { router.replace({ pathname: '/employer-job-details', params: { id: String(jobId) } }); return; }
      await load();
  ...
  return <SafeAreaView style={styles.container}><ScrollView contentContainerStyle={styles.content} refreshControl={<RefreshControl refreshing={refreshing} onRefresh={refreshStatus} tintColor={BrandColors.gold} />}>
    <View style={styles.header}><Pressable onPress={() => router.back()}><Text style={styles.back}>‹</Text></Pressable><View style={styles.headerCopy}><Text style={styles.eyebrow}>EMPLOYER BILLING</Text><Text style={styles.title}>Choose your plan</Text><Text style={styles.subtitle}>Post jobs using a monthly job allowance. No payment is required for each individual job.</Text></View></View>
    {subscription && <View style={styles.currentCard}><Text style={styles.currentEyebrow}>CURRENT SUBSCRIPTION</Text><Text style={styles.currentTitle}>{subscription.planName}</Text><Text style={styles.currentStatus}>{subscription.status.toUpperCase()}{subscription.cancelAtPeriodEnd ? ' • CANCELLING AT PERIOD END' : ''}</Text><View style={styles.usageRow}><Text style={styles.usage}>{subscription.jobsUsed} / {subscription.jobLimit} jobs used</Text><Text style={styles.usage}>₹{subscription.priceInr.toLocaleString('en-IN')} / {subscription.billingInterval.toLowerCase()}</Text></View><View style={styles.progressTrack}><View style={[styles.progress, { width: `${Math.min(100, (subscription.jobsUsed / Math.max(1, subscription.jobLimit)) * 100)}%` }]} /></View>{!active && <Text style={styles.warning}>Payment is not active yet. Complete the Razorpay authorisation and refresh status.</Text>}{active && jobId && <Pressable onPress={() => router.replace({ pathname: '/employer-job-details', params: { id: String(jobId) } })} style={styles.primary}><Text style={styles.primaryText}>Continue to publish job</Text></Pressable>}{subscription.status === 'active' && !subscription.cancelAtPeriodEnd && <Pressable onPress={cancel} style={styles.cancelButton}><Text style={styles.cancelText}>Cancel at period end</Text></Pressable>}</View>}
    ...
    {!subscription && plans.map((plan) => <View key={plan.code} style={styles.planCard}><View style={styles.planTop}><Text style={styles.planName}>{plan.name}</Text><Text style={styles.planPrice}>₹{plan.priceInr.toLocaleString('en-IN')}<Text style={styles.planPeriod}> / month</Text></Text></View><Text style={styles.planDescription}>{plan.description}</Text><View style={styles.featureRow}><Text style={styles.check}>✓</Text><Text style={styles.feature}>{plan.jobLimit} job postings per month</Text></View><View style={styles.featureRow}><Text style={styles.check}>✓</Text><Text style={styles.feature}>Recurring Razorpay billing</Text></View><Pressable disabled={Boolean(creating)} onPress={() => choosePlan(plan)} style={styles.primary}><Text style={styles.primaryText}>{creating === plan.code ? 'Opening payment...' : `Choose ${plan.name}`}</Text></Pressable></View>)}
