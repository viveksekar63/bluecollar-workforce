...
export default function EmployerNavigation() {
  ...
  return <>
    ...
    {!open && <View style={styles.bottomBar}>
      ...
      <Tab icon="▤" label="My Jobs" active={pathname === '/employer-jobs' || pathname === '/employer-ai-recruiter' || pathname === '/employer-recruitment-workspace'} onPress={() => navigate('/employer-jobs')} />
    ...
    {open && <View style={styles.layer} pointerEvents="box-none">
      ...
      <SafeAreaView style={styles.drawer}>
        <View style={styles.profileHeader}>
          ...
          <View style={styles.profileCopy}><Text style={styles.name} numberOfLines={1}>{displayName}</Text><Text style={styles.company} numberOfLines={1}>{employer?.companyName || 'Employer account'}</Text><Text style={styles.role}>EMPLOYER</Text></View>
          <Pressable onPress={() => setOpen(false)} style={styles.closeButton}><Text style={styles.close}>×</Text></Pressable>
        </View>
        <View style={styles.divider} />
        <NavItem icon="▤" label="My Jobs" onPress={() => navigate('/employer-jobs')} />
        <NavItem icon="✦" label="AI Recruiter" onPress={() => navigate('/employer-jobs')} />
        <NavItem icon="⌕" label="Find Manpower" onPress={() => navigate('/employer-find-manpower')} />
        ...
        <View style={styles.bottomArea}>
          {(user?.roles ?? []).includes('WORKER') && <NavItem icon="⇄" label="Switch to Worker" onPress={() => { setOpen(false); setActiveRole('WORKER'); router.replace('/home'); }} />}
          <Pressable style={styles.logout} onPress={logout}><Text style={styles.logoutIcon}>↪</Text><Text style={styles.logoutText}>Logout</Text></Pressable>
        </View>
      </SafeAreaView>
    </View>}
  </>;
...
function NavItem({ icon, label, onPress }: { icon?: string; label: string; onPress: () => void }) {
  return <Pressable style={({ pressed }) => [styles.item, pressed && styles.itemPressed]} onPress={onPress}><View style={styles.itemIcon}><Text style={styles.itemIconText}>{icon}</Text></View><Text style={styles.itemLabel}>{label}</Text><Text style={styles.chevron}>›</Text></Pressable>;
}

const styles = StyleSheet.create({
  menuButton: { position: 'absolute', top: Platform.OS === 'ios' ? 52 : 38, left: 20, zIndex: 150, width: 46, height: 46, borderRadius: 16, backgroundColor: '#FFFFFF', borderWidth: 1, borderColor: '#BFDBFE', alignItems: 'center', justifyContent: 'center', shadowColor: '#0A1F44', shadowOpacity: .12, shadowRadius: 10, shadowOffset: { width: 0, height: 4 }, elevation: 5 }, menuIcon: { color: BrandColors.indigo, fontSize: 24, fontWeight: '900', lineHeight: 25 },
  ...
  tab: { flex: 1, height: 70, alignItems: 'center', justifyContent: 'center', position: 'relative' }, tabIcon: { color: '#64748B', fontSize: 22, lineHeight: 28 }, tabIconActive: { color: BrandColors.indigo }, tabLabel: { color: '#64748B', fontSize: 8.5, fontWeight: '700', marginTop: 4 }, tabLabelActive: { color: BrandColors.indigo, fontWeight: '900' }, activeLine: { position: 'absolute', bottom: 1, width: 42, height: 3, borderRadius: 2, backgroundColor: BrandColors.indigo },
